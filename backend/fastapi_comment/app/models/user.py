...
class Comment(Base):
    ...
    id = Column(Integer, primary_key=True)
    user_id = Column(Integer, ForeignKey("myapp_user.id", ondelete="CASCADE"))
    test_id = Column(Integer, ForeignKey("myapp_testlar.id", ondelete="CASCADE"), nullable=True)
    text = Column(Text, nullable=True)
    created = Column(DateTime, default=func.now())
    type = Column(String(50), nullable=True)  # comment yoki reply
    parent_comment_id = Column(Integer, ForeignKey("myapp_comment.id", ondelete="CASCADE"), nullable=True)  # Agar reply bo'lsa, parent commentni saqlash uchun
...
class Aloqa(Base):
    __tablename__ = "myapp_aloqa"
    id = Column(Integer, primary_key=True)
    user_id = Column(Integer, ForeignKey("myapp_user.id", ondelete="CASCADE"), nullable=True)
